document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function loadLeaderboard(sortBy) {
        try {
            ...
            const response = await fetch(`http://localhost:5000/leaderboard?sortBy=${sortBy}&limit=100`);
            if (!response.ok) {
                    ...
                    <tr class="${isCurrentUser ? 'current-user-row' : ''}" data-username="${user.username}">
                        ...
                        <td class="user-cell">
                            <div class="user-info">
                                ...
                                ${isOrganizer ? '<span class="organizer-badge"><i class="bi bi-star-fill" style="color: #FFD700;"></i> Organizer</span>' : ''}
                                ${achievementBadges}
                            </div>
                        </td>
                        <td class="points-cell">
                            <span class="points-value">${user.points}</span> <i class="bi bi-trophy-fill" style="color: gold; font-size: 20px;"></i>
                        </td>
                        <td class="stat-cell">
                            ${isOrganizer ? `${user.eventsCreated} <i class="bi bi-calendar-check-fill" style="font-size: 18px;"></i>` : '-'}
                        </td>
                        <td class="stat-cell">${user.eventsAttended || 0} <i class="bi bi-ticket-perforated-fill" style="font-size: 18px;"></i></td>
                    </tr>
                `;
            })
            .join("");
    ...
    function getRankIcon(rank) {
        // Modern numbered badges for top 3 ranks
        switch(rank) {
            case 1: return '<span class="rank-badge-modern rank-1-modern">1st</span>';
            case 2: return '<span class="rank-badge-modern rank-2-modern">2nd</span>';
            case 3: return '<span class="rank-badge-modern rank-3-modern">3rd</span>';
            default: return '';
        }
    }
