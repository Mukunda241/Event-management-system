...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        ...
        // Initialize FullCalendar
            // Ensure FullCalendar is available (try multiple CDNs, with graceful fallback)
            async function ensureFullCalendarLoaded() {
                if (window.FullCalendar) return;

                // Prefer jsDelivr first (more reliable), then cdnjs, then unpkg
                const cdns = [
                    {
                    ...
                    {
                        css: 'https://cdnjs.cloudflare.com/ajax/libs/fullcalendar/5.11.3/main.min.css',
                        js: 'https://cdnjs.cloudflare.com/ajax/libs/fullcalendar/5.11.3/main.min.js'
                    },
                    {
                        css: 'https://unpkg.com/@fullcalendar/core@5.11.3/main.min.css',
                        js: 'https://unpkg.com/@fullcalendar/core@5.11.3/main.min.js'
                    }
                ];

                for (const source of cdns) {
                    try {
                        // Load CSS if missing
                        if (![...document.styleSheets].some(s => (s.href || '').includes('fullcalendar'))) {
                        ...
                        await new Promise((resolve, reject) => {
                            ...
                            script.onerror = () => reject(new Error(`Failed to load script from ${source.js}`));
                            document.head.appendChild(script);
                        });

                        if (window.FullCalendar) {
                            console.log(`✅ Loaded FullCalendar from ${source.js}`);
                            return;
                        }
                    } catch (err) {
                        // Log at info level to avoid alarming warnings when CDNs are flaky
                        console.info('FullCalendar CDN failed, trying next source...', err.message || err);
                        // try next CDN
                    }
                }
                // Last-ditch: try a local copy under /libs/fullcalendar/ (you can drop the minified files there)
                const localCss = '/libs/fullcalendar/main.min.css';
                const localJs = '/libs/fullcalendar/main.min.js';
                try {
                    if (![...document.styleSheets].some(s => (s.href || '').includes('fullcalendar'))) {
                        const link = document.createElement('link');
                        link.rel = 'stylesheet';
                    ...
                    await new Promise((resolve, reject) => {
                        ...
                        script.async = false;
                        script.onload = () => resolve();
                        script.onerror = () => reject(new Error(`Failed to load local script from ${localJs}`));
                        document.head.appendChild(script);
                    });
                    if (window.FullCalendar) {
                        console.log(`✅ Loaded FullCalendar from local file ${localJs}`);
                        return;
                    }
                } catch (localErr) {
                    console.warn('⚠️ Local FullCalendar fallback failed', localErr.message || localErr);
                }

                throw new Error('All FullCalendar CDN sources failed');
            }

            let fcAvailable = true;
            try {
                await ensureFullCalendarLoaded();
            } catch (loadErr) {
                console.error('❌ Could not load FullCalendar:', loadErr);
                fcAvailable = false;
            }
            ...
            if (!fcAvailable) {
                // Graceful fallback: show a simple list view in the calendar area so users can still access events
                if (calendarEl) {
                    calendarEl.innerHTML = '<div class="calendar-unavailable" style="padding:16px;border:1px dashed #e5e7eb;border-radius:8px;background:#fff8f0;color:#92400e;margin-bottom:12px;">⚠️ Calendar failed to load from CDN. Showing simple list view below.</div>';
                    const ul = document.createElement('ul');
                    ul.className = 'fallback-event-list';
                    ul.style.listStyle = 'none';
                    ul.style.padding = '0';
                    ul.innerHTML = allEvents.map(ev => `
                        <li style="padding:8px 0;border-bottom:1px solid #f3f4f6;">
                            <a href="event-template.html?event=${encodeURIComponent(ev.name)}" style="color:#111827;text-decoration:none;font-weight:600;">${ev.name}</a>
                            <div style="color:#6b7280;font-size:13px;">${ev.date} • ${ev.venue || 'Venue TBA'}</div>
                        </li>
                    `).join('');
                ...
                return; // stop trying to initialize FullCalendar
            }
        calendar = new FullCalendar.Calendar(calendarEl, {
            initialView: 'dayGridMonth',
            height: 'auto', // let FullCalendar size itself to container width/height
            headerToolbar: {
                // Minimal toolbar: arrows on left, title centered, view buttons small on right
                left: 'prev,next',
                center: 'title',
            ...
            dateClick: function(info) {
                ...
                // Highlight and render selected date events
    ...
    if (eventList) {
        eventList.innerHTML = events.map(event => `
            <li data-date="${event.date}">
                ...
                <div class="event-meta">${event.date} • ${event.venue || 'Venue TBA'}</div>
            </li>
        `).join("");
    }
}
