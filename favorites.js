document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function createEventCard(event) {
        ...
        if (loggedInUser && loggedInUser.username) {
            try {
                const response = await fetch(`http://localhost:5000/users/${loggedInUser.username}/pinned`);
                if (response.ok) {
    ...
    async function loadFavorites() {
        ...
        if (filteredEvents.length === 0) {
            favoriteEventsGrid.style.display = "none";
            if (favoritesEmpty) {
                favoritesEmpty.style.display = "block";
            }
        } else {
            favoriteEventsGrid.style.display = "grid";
            if (favoritesEmpty) {
    ...
    window.removeFromFavorites = async function (eventId) {
        ...
        try {
            const response = await fetch(`http://localhost:5000/users/${loggedInUser.username}/favorites/${eventId}`, {
                method: 'DELETE'
            });
    ...
    window.togglePin = async function (eventId) {
        ...
        try {
            ...
            if (index === -1) {
                ...
                if (eventToPin) {
                    const response = await fetch(`http://localhost:5000/users/${loggedInUser.username}/pinned/${eventId}`, {
                        method: 'POST'
                    });
