...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        // Load nearby hotels and transportation if coordinates are available
        if (event.lat && event.lng) {
            console.log('📍 Loading nearby hotels and transportation for coordinates:', event.lat, event.lng);
            
            // Load nearby hotels
            if (typeof loadNearbyHotels === 'function') {
                loadNearbyHotels(event.lat, event.lng, event.name);
            } else {
                // Retry after a delay if not loaded yet
                setTimeout(() => {
                    if (typeof loadNearbyHotels === 'function') {
                        loadNearbyHotels(event.lat, event.lng, event.name);
                    }
            ...
            // Load nearby transportation
            if (typeof loadNearbyTransportation === 'function') {
            ...
            console.log('ℹ️ No coordinates available for event - hotels and transportation sections will not be shown');
        }
    ...
    speakersGrid.innerHTML = speakers.map(speaker => `
        <div class="speaker-card">
            <div class="speaker-photo-container">
                ...
                <span class="speaker-role-badge">${speaker.role || 'Speaker'}</span>
            </div>
            ...
            <div class="speaker-info">
                ...
                ${(speaker.socialLinks && (speaker.socialLinks.linkedin || speaker.socialLinks.twitter || speaker.socialLinks.website)) ? `
                    <div class="speaker-social">
                        ${speaker.socialLinks.linkedin ? `<a href="${speaker.socialLinks.linkedin}" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>` : ''}
                        ${speaker.socialLinks.twitter ? `<a href="${speaker.socialLinks.twitter}" target="_blank" title="Twitter"><i class="fab fa-twitter"></i></a>` : ''}
                        ${speaker.socialLinks.website ? `<a href="${speaker.socialLinks.website}" target="_blank" title="Website"><i class="fas fa-globe"></i></a>` : ''}
                    </div>
                ` : ''}
            </div>
        </div>
...
function initializeRegistration(event) {
    ...
    async function updateRegistrationUI() {
        ...
        if (username) {
            try {
                const response = await fetch(`${API_CONFIG.BASE_URL}/tickets/${username}`);
                if (response.ok) {
    ...
    cancelTicketsBtn.addEventListener("click", async () => {
        ...
        showCancelConfirmation(event.name, async () => {
            ...
            try {
                ...
                const response = await fetch(`${API_CONFIG.BASE_URL}/tickets/${event._id}/${username}`, {
                    method: 'DELETE',
                    headers: {
...
document.getElementById("proceedToCheckout").addEventListener("click", async () => {
    ...
    try {
        const response = await fetch(`${API_CONFIG.BASE_URL}/events/${currentEvent._id}/register`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function updateAttendeeCount(event) {
    try {
        ...
        const response = await fetch(`${API_CONFIG.BASE_URL}/events/${event._id}`);
        if (response.ok) {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        if (loggedInUser && loggedInUser.username) {
            try {
                const favoritesResponse = await fetch(`${API_CONFIG.BASE_URL}/users/${loggedInUser.username}/favorites`);
        ...
        favoriteBtn.addEventListener("click", async () => {
            ...
            try {
                if (index === -1) {
                    const response = await fetch(`${API_CONFIG.BASE_URL}/users/${loggedInUser.username}/favorites/${event._id}`, {
                        method: 'POST'
                    });
        ...
        pinBtn.addEventListener("click", async () => {
            ...
            try {
                if (index === -1) {
                    const response = await fetch(`${API_CONFIG.BASE_URL}/users/${loggedInUser.username}/pinned/${event._id}`, {
                        method: 'POST'
                    });
