...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    try {
      const pinnedResponse = await fetch(`${API_CONFIG.BASE_URL}/users/${username}/pinned`);
      if (pinnedResponse.ok) {
        pinnedEvents = await pinnedResponse.json();
      }
      
      const favoritesResponse = await fetch(`${API_CONFIG.BASE_URL}/users/${username}/favorites`);
    ...
    window.toggleFavorite = async function (eventId) {
      ...
      try {
        if (index === -1) {
          // Add to favorites
          const response = await fetch(`${API_CONFIG.BASE_URL}/users/${username}/favorites/${eventId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
          });
    ...
    window.togglePin = async function (eventId) {
      ...
      try {
        if (index === -1) {
          // Pin event
          const response = await fetch(`${API_CONFIG.BASE_URL}/users/${username}/pinned/${eventId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
          });
