...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('🔍 DEBUG: User loaded in My Events:', currentUser);
...
async function displayMyEvents() {
    ...
    try {
        ...
        console.log('🔍 DEBUG: All events:', events.map(e => ({ name: e.name, organizer: e.organizer, status: e.status })));
        console.log('🔍 DEBUG: Looking for events by organizer:', currentUser.username);

        // Filter events created by current user
        const myEvents = events.filter(event => event.organizer === currentUser.username);
        
        console.log('🔍 DEBUG: My events found:', myEvents.length);
        console.log('🔍 DEBUG: My events details:', myEvents.map(e => ({ name: e.name, status: e.status })));
