document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function loadLeaderboard(sortBy) {
        try {
            ...
            if (loggedInUser) {
                ...
                await showUserRank(loggedInUser.username, sortBy);
            }
        } catch (error) {
    ...
    async function showUserRank(username, sortBy) {
        try {
            const response = await fetch(`${API_CONFIG.BASE_URL}/users/${username}/rank?sortBy=${sortBy}`);
            if (response.ok) {
                const data = await response.json();
                ...
                // Determine label and value based on sortBy
                let valueLabel = 'Points';
                let valueDisplay = `${data.points} <i class="fas fa-trophy" style="color: gold;"></i>`;
                
                if (sortBy === 'eventsCreated') {
                    valueLabel = 'Events Created';
                    valueDisplay = `${data.eventsCreated} <i class="fas fa-calendar-check"></i>`;
                } else if (sortBy === 'eventsAttended') {
                    valueLabel = 'Events Attended';
                    valueDisplay = `${data.eventsAttended} <i class="fas fa-ticket-alt"></i>`;
                }
                ...
                rankInfo.innerHTML = `
                    <div class="rank-card">
                        ...
                        <div class="rank-details">
                            ...
                            <div class="rank-item">
                                <span class="rank-label">${valueLabel}</span>
