...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function loadTickets() {
        try {
            const response = await fetch(`${API_CONFIG.BASE_URL}/tickets/${loggedInUser.username}`);
            if (!response.ok) {
    ...
    window.cancelTicket = async function(eventId, eventName) {
        ...
        showCancelConfirmation(eventName, async () => {
            try {
                ...
                const response = await fetch(`${API_CONFIG.BASE_URL}/tickets/${eventId}/${loggedInUser.username}`, {
                    method: 'DELETE'
                });
