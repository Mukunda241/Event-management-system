...
class NotificationManager {
  ...
  async loadUnreadCount() {
    try {
      const response = await fetch(`/api/notifications/${this.username}/unread-count`);
  ...
  async loadNotifications(page = 1) {
    ...
    try {
      const response = await fetch(`/api/notifications/${this.username}?page=${page}&limit=20`);
      const data = await response.json();
  ...
  async markAllAsRead() {
    try {
      await fetch(`/api/notifications/${this.username}/read-all`, {
        method: 'PUT'
      });
