...
// Serve home.html at root path BEFORE static middleware
app.get("/", (req, res) => {
  res.sendFile(path.join(__dirname, "home.html"));
});

// Static files (now also allows /index.html, /login.html, scripts, css etc)
...
app.delete("/events/:id/register", async (req, res) => {
  try {
    ...
    if (event.status === 'Closed' && event.registeredUsers.length < event.capacity) {
      event.status = 'Active';
    }
    ...
    const eventsWithStats = events.map(event => {
      ...
      return {
        ...
        statistics: {
          ...
          isFull: totalBookings >= event.capacity
        },
        recentRegistrations
      };
    });
...
app.get("/events/:id/bookings", async (req, res) => {
  try {
    ...
    const stats = {
      ...
      availableSeats: event.capacity - bookings.length,
      capacityUsed: ((bookings.length / event.capacity) * 100).toFixed(1)
    };
